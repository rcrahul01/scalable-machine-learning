...
firstPointFeatures = map(lambda x : x.features.toArray(),parsedSamplePoints)[0]
firstPointLabel = map(lambda x : x.label,parsedSamplePoints)[0]
...
def calcRMSE(labelsAndPreds):
    ...
    """
    labelsAndPreds2 = labelsAndPreds.collect()
    
    return math.sqrt(sum([squaredError(x[0],x[1]) for x in labelsAndPreds2])/len(labelsAndPreds2))
